...
<tr><!-- 첫번째 줄 시작 -->
<td>
$a< x \le b$인 $x$에 대하여 닫힌구간 $[a, x]$에서 $\fbox{\phantom{eu}(가)\phantom{eu}}$에 따라 $\dfrac{f(x)-f(a)}{x-a}=f'(c)$인 $c$가 열린구간 $(a, \fbox{\phantom{eu}(가)\phantom{eu}})$에 적어도 하나 존재한다. 이때, $f'(c)=\fbox{\phantom{eu}(가)\phantom{eu}}$이므로 $\dfrac{f(x)-f(a)}{x-a}=0$ 즉, $f(x)-f(a)=0$이므로 $f(x)=f(a)$ 따라서 함수 $f(x)$는 닫힌구간 $[a, x]$에서 상수함수이다.
</td>
</tr><!-- 첫번째 줄 끝 -->
</table>
