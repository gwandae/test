...
|조건|
|-|
|(가) $\lvert a _9\rvert+\lvert a_{10}\rvert=\lvert a_9+a_{10}\rvert+4$|
|(나) $\lvert a_1\rvert<10$|
|(다) $\displaystyle\sum _{n=1}^{10}(a_ n+\lvert a_n\rvert)=54$|
...
좌표평면에서 자연수 $n$에 대하여 $A_n$을 $4$개의 점 $(n^2, n^2), (4 n^2, n^2), (4 n^2, 4 n^2), (n^2, 4n^2)$을 꼭짓점으로 하는 정사각형이라 하자. 정사각형 $A_n$과 함수 $y=k\sqrt x$의 궁그래프가 만나도록 하는 자연수 $k$의 개수를 $a_n$이라 할 때, $\displaystyle\sum_{m=1}^{10}(a_{3m-1}+a_{3m-2})$의 값을 구하면? [6점]
